...
class Input extends React.PureComponent {
    ...
    return (
      <Consumer>
        {(context) => {
          ...
          const { title, name, type, prepend, append, maxlength, disabled, readonly, autofocus } =
            this.props;
          ...
          return (
            <div className={this.wrapCls} ref={this.inputWrapRef}>
              <div className={this.innerCls}>
                {title && (
                  <label className={this.labelCls} style={this.labelStyle}>
                    {title}
                  </label>
                )}
                {prepend && <div className={this.prependCls}>{prepend}</div>}
                <div className={this.contentCls}>
                  ...
                  />
                  {this.showEditIcon && (
                    <div className={classNames(['input-edit', this.inputCls])}>
                      <span className="placeholder">{value}</span>
                      <span className="edit-icon"></span>
                    </div>
                  )}
                  {this.showClear && (
                    <span className={this.clearCls} onClick={this.onClear}>
                      <svg
                        width="14px"
                        height="14px"
                ...
                {append && <div className={this.appendCls}>{append}</div>}
                {this.showErrorMsg && <div className={this.errorCls}>{validateMessage}</div>}
              </div>
              {this.showEmailPanel && (
                <div className={this.emailPanel}>
                  {this.filteredEmailList.map((emailSuffix, idx) => (
                    <div
                      className={this.emailPanelItem}
                      key={idx}
                      onClick={this.onEmailClick}
                      dangerouslySetInnerHTML={{
                        __html: (this.state.value.split('@')[0] + '@' + emailSuffix).replace(
                          this.state.value,
                          function (text) {
  ...
  get showErrorMsg() {
    return (
      this.state.validateState === 'error' &&
  ...
  onFieldInput = (e) => {
    ...
    if (type === 'number') {
      ...
      if (dotPos > -1) {
        e.target.value =
          fixed > 0 ? iptValue.substring(0, dotPos + fixed + 1) : iptValue.substring(0, dotPos);
      }
  ...
  validate(trigger, callback = function () {}) {
    ...
    const isMaskCode =
      initialValue.indexOf && initialValue.indexOf('*') > -1 && initialValue === value; // 掩码
    ...
    validator
      .validate(model, { first: true, suppressWarning: true, component: this })
      .then(() => {
